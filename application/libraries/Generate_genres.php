...
class Generate_genres {
	...
	public function get_tags($class = null, $limit = 5, $start = 0, $use_count = true){
		...
		if($count <= 0){
			...
			foreach($sql as $genre):
				...
				$content .= '<li><a href="'.$config->myurl.'genre/'.strtolower($data['namegenre']).'/" rel="tag" title="'.$data['namegenre'].'" '.$class.'>'.$genre_count.'</a></li>';	
			endforeach;
			}
			return $content;
		}
	...
	function get_tag_link($tags,$tagcount){
		...
		$num = 1;
		foreach ($tags as $tagname):
			$arrTag[] = '<a href="'.base_url().'genre/'.strtolower($tagname).'/" rel="tag" target="_BLANK">'.$tagname.'</a>';
		endforeach;
